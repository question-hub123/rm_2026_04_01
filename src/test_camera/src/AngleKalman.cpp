...
AngleKalman::AngleKalman(double dt)
    : initialized_(false)
{
...
void AngleKalman::init(double yaw, double pitch)
{
...
void AngleKalman::predict(double dt)
{
...
double AngleKalman::normalizeAngle(double angle)
{
...
void AngleKalman::update(double yaw_meas, double pitch_meas)
{
    if (!initialized_) return;
...
void AngleKalman::getState(double& yaw, double& yaw_rate, double& pitch, double& pitch_rate) const
{
    yaw = X_(0);
    yaw_rate = X_(1);
    pitch = X_(2);
